...
import Step5Success from "@/components/reserva/Step5Success";
...
export default function ReservaPage() {
  ...
  const handleStep3Next = (guestData: {
    guestName: string;
    guestEmail: string;
    guestPhone: string;
    guestNotes: string;
  ...
  const handleResetFlow = () => {
    // Limpiamos los datos y volvemos al paso 1
    ...
    <div className="min-h-screen bg-[#94a3b8] flex justify-center sm:py-8">
      ...
      <div className="w-full max-w-md bg-white sm:rounded-[2.5rem] shadow-2xl relative flex flex-col min-h-screen sm:min-h-[800px] overflow-hidden">
        ...
        {currentStep < 5 ? (
          <>
            ...
            <main className="flex-1 px-6 pt-12 pb-8 flex flex-col h-full">
              ...
              <div className="flex-1 flex flex-col h-full">
                {currentStep === 1 && (
                  <Step1EventSelection onNext={handleStep1Next} />
                )}
                {currentStep === 2 && (
                  <Step2DateSelection
                    eventDuration={
                      eventsStore.find((e) => e.id === reservaData.eventId)
                        ?.duration || 30
                    }
                ...
                {currentStep === 3 && (
                  <Step3GuestData onNext={handleStep3Next} />
                )}
                {currentStep === 4 && (
                  <Step4Confirmation
          ...
          <Step5Success onReset={handleResetFlow} />
        )}
      </div>
    </div>
  );
